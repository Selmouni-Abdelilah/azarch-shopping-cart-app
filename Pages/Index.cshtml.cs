...
namespace cart.Pages
{
    public class IndexModel : PageModel
    {
        ...
        public IndexModel(ILogger<IndexModel> logger, IConfiguration config , MenuContext context)
        {
            _logger = logger;
            _context=context;   
            _config=config;        
        ...
        public void OnGet()
        {
            MenuLoader.LoadMenus(_context);
            List<Menu> menus=new List<Menu>();
            // UNCOMMENT AFTER WE HAVE A DATABASE  
            menus=GetMenusInShoppingCart();
            Console.WriteLine("Number of items in shopping cart: " +menus.Count); 
        ...
        public IActionResult OnPostRemoveFromShoppingCart()  {
            ...
            client.RemoveItemFromList("cart", menuId.ToString());            
        ...
        public void OnPostPlaceOrder()  {
            ...
            var menusInCart=GetMenusInShoppingCart();
            var order=new Order();
            double total=0;
            order.orderDate=DateTime.Now;
            order.items=new List<OrderItem>();

            foreach (var menu in menusInCart)  {
        ...
        private List<Menu> GetMenusInShoppingCart()  {

            var client=GetRedisClient();
            var menuIDs=client.GetAllItemsFromList("cart");                 
            return _context.Menus.Where(b=>menuIDs.Contains(b.ID.ToString())).ToList();
        }
